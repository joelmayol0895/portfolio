import Figma from '@/assets/images/figma.webp'
import Html from '@/assets/images/html.webp'
import Css from '@/assets/images/css-3.webp'
import Js from '@/assets/images/js.webp'
import React from '@/assets/images/react.webp'
import TailWind from '@/assets/images/wind.webp'
import Php from '@/assets/images/php.webp'
import WordPress from '@/assets/images/wordpress.webp'
import Git from '@/assets/images/git.webp'
import Sass from '@/assets/images/sass.webp'
import Bootstrap from '@/assets/images/bootstrap.webp'
import Github from '@/assets/images/github.webp'

const SkillsData = [
    {
      label: "Figma",
      caption: "Design Tool",
      icon: Figma,
    },
    {
      label: "HTML5",
      caption: "Markup Language",
      icon: Html,
    },
    {
      label: "CSS3",
      caption: "Styling Language",
      icon: Css,
    },
    {
      label: "Javascript",
      caption: "Scripting / Interaction",
      icon: Js,
    },
    {
      label: "React",
      caption: "JS Framework",
      icon: React,
    },
    {
      label: "Tailwind CSS",
      caption: "Utility-First CSS Framework",
      icon: TailWind,
    },
    {
      label: "WordPress",
      caption: "Content Management System",
      icon: WordPress,
    },
    {
      label: "PHP",
      caption: "Server-Side Scripting",
      icon: Php,
    },
    {
      label: "Git",
      caption: "Version Control",
      icon: Git,
    },
    {
      label: "Sass",
      caption: "CSS Preprocessor",
      icon: Sass,
    },
    {
      label: "Bootstrap",
      caption: "CSS Framework",
      icon: Bootstrap,
    },
    {
      label: "GitHub",
      caption: "Code Hosting Platform",
      icon: Github,
    },
];
  
export default SkillsData;
  
