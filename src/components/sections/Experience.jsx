...
const Experience = () => {
  ...
  return (
    <section id="experience">
        <div className='container min-h-auto flex flex-col py-10 md:py-20 px-5 sm:px-10 text-center space-y-9'>
            <h2 className='text-2xl md:text-4xl font-bold tracking-tight mb-7'>My <span className='text-primary'>Journey</span></h2>
            <div>
                <h3 className='text-2xl md:text-3xl font-bold mb-15'><span className='text-primary-foreground'>Work</span> Experience</h3>
                        ...
                        <div className={`flex flex-row justify-around items-start ${isExpand === key ? "h-full" : "overflow-hidden"}`} key={key}>
                            ...
                            <div className={`w-full lg:w-[70%] border-l-2 border-primary-foreground py-3 md:py-5 pl-5 sm:pl-8 text-left relative`}>
                                <div className="flex flex-row items-center p-4 bg-linear-to-l from-background/80 to-background-secondary rounded-md">
                                    ...
                                    <h4 className='text-[16px] sm:text-xl md:text-2xl font-semibold cursor-pointer text-foreground/80' onClick={() => toggleExpand(key)}>
                                        {exp.company}
                                        <span className="text-[14px] sm:text-md md:text-xl text-primary-foreground block lg:hidden"> [ {exp.year} ]</span>
                                    </h4>
                                </div>
                                ...
                                <div key={i} className={`absolute bottom-0 transition-all duration-500 ease-in px-2 rounded-lg mt-3 
                                    ...
                                    <h5 className='text-[16px] sm:text-xl font-semibold mb-3'>
                                        {role.position} - <span className="text-primary inline-block text-[14px] sm:text-[16px]">[ {role.year} ]</span>
                                    </h5>
